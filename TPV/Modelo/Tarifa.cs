...
namespace TPV.Modelo
{
    public class Tarifa
    {
        ...
        public void AnadirProducto(Producto producto, double precioConIva, bool persistencia)
        {
            ...
            if (persistencia)
            {
                if (ControladorComun.BD!.BuscarObjetosIntAndInt<ObjProdPrecio>("CodProducto", producto.CodProducto, "CodTarifa", this._codTarifa).Count < 1) //Si no existía el producto en la tarifa   
                {
                    ControladorComun.BD!.PersistirObjeto(obj); //Lo persistimos
                }
                ControladorComun.BD!.PersistirObjeto(this); //Persistimos la tarifa 
            }
        }
        ...
        public bool EliminarProducto(int codProducto, bool persistencia)
        {
            ...
            if (this._listaProductos!.Remove(_listaProductos.Find(x => x.CodProducto == codProducto)!))
            {
                if (persistencia)
                {
                    ControladorComun.BD!.EliminarObjeto<ObjProdPrecio>(_listaProductos.Find(x => x.CodProducto == codProducto)!);
        ...
        public bool EliminarPrecioProducto(Producto producto, bool persistencia)
        {
            if (this._listaProductos!.Remove(_listaProductos.Find(x => x.CodProducto == producto.CodProducto)!))
            {
                if (persistencia)
                {
                    ControladorComun.BD!.EliminarObjeto<ObjProdPrecio>(_listaProductos.Find(x => x.CodProducto == producto.CodProducto)!);
                    ControladorComun.BD!.ActualizarObjeto(this);
                }
                return true;
            }
        ...
        public void ModificarPrecioProducto(int codProducto, double precio, bool persistencia)
        {
            ...
            if (persistencia)
            {
                ControladorComun.BD!.ActualizarObjeto(obj);
                ControladorComun.BD!.ActualizarObjeto(this);
            }
        }
